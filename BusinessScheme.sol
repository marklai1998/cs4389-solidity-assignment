...
interface I_BusinessOutlet{
    function createScheme(address[] calldata businessOutlets, string calldata schemeName, string calldata schemeContent, uint agreeVoteThreshold, uint rewardRatio) external returns (address schemeAddress);
    ...
    function payBillByRewards (uint points) external returns (bool status);
}

interface I_Scheme{
    function createScheme(address[] calldata businessOutlets, string calldata schemeName, string calldata schemeContent, uint schemeThreshold, uint schemeRatio) external;
...
contract BusinessOutlet is I_BusinessOutlet{
    ...
    function createScheme(address[] calldata businessOutlets, string calldata schemeName, string calldata schemeContent, uint agreeVoteThreshold, uint rewardRatio) external returns (address schemeAddress){
        ...
        newScheme.createScheme(businessOutlets, schemeName, schemeContent, agreeVoteThreshold, rewardRatio);
    ...
    function schemeInvitation(address payable schemeAddress) external {
        ...
        for(uint i = 0; i < invitedSchemeList.length; i++){
...
contract Scheme is I_Scheme{
    ...
    function createScheme(address[] calldata businessOutlets, string calldata schemeName, string calldata schemeContent, uint schemeThreshold, uint schemeRatio) external {
        ...
        ratio = schemeRatio;
        for(uint i = 0; i < businessOutlets.length; i++){
            ...
            bool alreadyExist = false;
            for(uint x = 0; x < invitedOutletList.length; x++){
    ...
    function createConsensus(myRewards rw) external {
        ...
        for(uint i = 0; i < invitedOutletList.length; i++){
...
contract myRewards is I_Rewards{
    ...
    modifier onlyInOutletList {
        bool alreadyExist = false;
        for(uint i = 0; i < outlet_list.length; i++){
    ...
    function earnRewards(address current_client, uint spending) external onlyInOutletList returns (bool status) {
        bool alreadyExist = false;
    ...
    function redeemRewards(address current_client, uint points) external onlyInOutletList returns (bool status) {
    ...
    function setup(address[] calldata businessOutlets, uint ratio) external{
...
contract TestCase {
    ...
    function Test01() public payable {
        ...
        address _s =  b0.createScheme(partners, "Joint Promotion Plan 1", 
                                  "Earn Points from our Partners! Earning one point for every 8 dollars spending!",2, 8);
